...
class atm:
  def __init__(self):
    ...
    # Read in pins from ssATM.bin
    fdSSatm = open("ssATM.bin", "r")
    lstATMLines = fdSSatm.readlines()
    self.aesKey = lstATMLines[0].strip()
    del lstATMLines[0]

    self.dictUserData = {}
    for user in lstATMLines:
        lstInfo = user.strip().split(";")
        self.dictUserData[str(lstInfo[0])] = str(lstInfo[1])
  ...
  def handleLocal(self,inString):
    ...
    if inString == "begin-session":
        ...
        with open("inserted.card", 'r') as insertedCard:
            ...
            user = lstCardData[0].lower()
            correctPin = str(self.dictUserData[user])

        # Authenticate the user by asking for pin
        pinAttempt = input("Enter PIN: ")
        print("Your attempt is: ", pinAttempt)
        if pinAttempt == correctPin: # Correct pin was entered
