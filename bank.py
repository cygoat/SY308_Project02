...
class bank:
  def __init__(self):
    ...
    for user in lstUsers:
        ...
        self.accounts[str(lstInfo[0])] = int(lstInfo[2])
      ...
      if command[0] == "deposit":
          ...
          while True:
              try:
                  ...
                  user = user.title()
                  output = "Deposited " + str(abs(amt)) + " to " + user + "'s account"
                  return output
          ...
          while True:
              try:
                  if self._isLocal == 1:
                      return "Error: Cannot withdraw from bank locally!"
                  if len(command) == 2: # User is logged in
                    user = self.currentUser.lower()
                    amt = int(command[1])
  ...
  def handleRemote(self, inBytes):
    ...
    atmCommand = str(decInput)[2:-1]
    # ATM command is not printed on the bank's terminal,
    # and is only sent back to the ATM for display.
    ...
    while True:
        try:
            ...
            self.sendBytes(encCommandOutput) #encode the message as bytes
            break
